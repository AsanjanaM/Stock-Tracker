...
option = st.sidebar.selectbox('Select one symbol', ( 'MSFT', 'MU','SPY', 'WMT', 'GME',  'NFLX', 'BNOX','AAPL',))
...
# Create a figure and plot today's price, open price, and close price in one graph with different colors
fig3, ax3 = plt.subplots(figsize=(12, 6))

current_price = yf.Ticker(option).history(period='1d')['Close'].iloc[0]

# Plot today's price as a red line
ax3.plot(stock_data.index, [current_price] * len(stock_data), label="Today's Price", color="#FF1493", marker='o')

# Plot the open price as a green line
...
ax.plot(stock_data.index, stock_data['Close'], label='Todays Price', color='#800000', marker='o')
# Add labels and title
ax.set_xlabel('Date')
ax.set_ylabel('Price (USD)')
ax.set_title(f'Todays Price Comparison for {option}')
ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=1))
ax.xaxis.set_major_formatter(mdates.DateFormatter("%m-%d"))
# Display legend
plt.legend()
...
# Create another figure and plot the predicted price in a separate graph
...
prediction_message = ""
prediction_color = "#E9967A"  # Default color for prediction message
...
if predictions[-1] > current_price:
    ...
    prediction_symbol = "↓"  # Down symbol
else:
    prediction_message = "Predicted Price is the Same as Today's Price"
    prediction_symbol = "→"  # No change symbol

# Display the prediction message with the appropriate color and style
st.markdown("<h1 style='color:#FFE4E1; font-family:Gabriola, Times, serif;'>PREDICITION PRICE</h1>", unsafe_allow_html=True)
...
style = f"{box_style} background-color: {prediction_color};"
